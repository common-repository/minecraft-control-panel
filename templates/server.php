...
if (!empty($data[0]['success'])) {
    foreach($data[0]['success'] as $consK => $consV) {
        $consLines .= '<tr><td>' . $mcpf->stripColor($data[0]['success'][$consK]['line'], 'hex') . '</td></tr>';    
    }
}    
...
if (!empty($data[1]['success'])) {
    foreach($data[1]['success'] as $chatK => $chatV) {
        $chatLines .= '
            <tr>
                <td>
                    ' . date("Y-m-d H:i:s", $data[1]['success'][$chatK]['time']) . ' [' . ($data[1]['success'][$chatK]['player'] ? $data[1]['success'][$chatK]['player'] : 'Konsole'). '] ' . $mcpf->stripColor($data[1]['success'][$chatK]['message'], 'hex') . '
                </td>
            </tr>
        ';    
    }    
...
<div class="mcpanel wrap">
    <h2 class="mcp">Server Verwaltung</h2>
